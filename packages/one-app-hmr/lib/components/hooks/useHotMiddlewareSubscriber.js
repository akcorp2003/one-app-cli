...
export function logModuleLoad(Module) {
  console.log(['%c', `${libraryName} :: Hot Holocron module "${Module.moduleName}" has been loaded.`].join(' '), 'background: #222; color: #bada55');
}
    ...
    switch (payload.action) {
      ...
      case 'locale:remove':
        if (payload.moduleName === Module.moduleName) {
          ...
          dispatch(loadLanguagePack(payload.moduleName, payload.locale, true, 'en-US'))
